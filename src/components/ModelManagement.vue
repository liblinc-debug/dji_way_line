...
const form = reactive({ modelCode: '', modelName: '', vendor: '', supports_gimbal: true, supports_actuator: false })

const filteredModels = computed(() => {
  const query = keyword.value.trim().toLowerCase()
  if (!query) return models.value
  return models.value.filter((item) => [item.modelCode, item.modelName, item.vendor].some((value) => String(value || '').toLowerCase().includes(query)))
})
...
function resetForm() {
  Object.assign(form, { modelCode: '', modelName: '', vendor: '', supports_gimbal: true, supports_actuator: false })
}
...
async function createModel() {
  ...
  errorText.value = ''
  try {
    await taskApiRequest('/models', {
      method: 'POST',
      body: JSON.stringify({
        modelCode: form.modelCode.trim(),
        modelName: form.modelName.trim(),
        vendor: form.vendor.trim(),
        capabilities: { supports_gimbal: form.supports_gimbal, supports_actuator: form.supports_actuator }
      })
    })
    message.success(`机型 ${form.modelCode} 已创建`)
    resetForm()
    await loadData()
  } catch (error) {
    errorText.value = error.message
    ...
    <div class="management-scroll">
      ...
      <div class="management-layout">
        <section class="management-panel list-panel">
          <div class="management-toolbar">
            ...
            <div class="toolbar-actions">
              <a-input-search v-model:value="keyword" allow-clear placeholder="搜索编码、名称或厂商" />
          ...
          <div class="resource-table-wrap">
            <table class="resource-table">
              <thead><tr><th class="check-cell"></th><th>机型</th><th>厂商</th><th>能力摘要</th><th>关联飞机</th><th class="action-cell">操作</th></tr></thead>
              <tbody>
                <tr v-for="model in filteredModels" :key="model.modelCode">
                  ...
                  <td class="action-cell"><a-button type="link" danger @click="removeModel(model)">删除</a-button></td>
                </tr>
                <tr v-if="!filteredModels.length"><td colspan="6"><a-empty description="暂无匹配机型" /></td></tr>
              </tbody>
            </table>
          </div>
        </section>
        ...
        <aside class="management-panel form-panel">
          <div class="form-heading"><span class="form-icon">＋</span><div><h2>新增机型</h2><p>创建可复用的飞机能力模板</p></div></div>
          <div class="form-body">
            <label>机型编码 <b>*</b></label><a-input v-model:value="form.modelCode" placeholder="例如 DJI-M350" />
            ...
            <a-button block type="primary" size="large" :loading="saving" @click="createModel">创建机型模板</a-button>
            <a-button block @click="resetForm">清空表单</a-button>
          </div>
        </aside>
      </div>
    </div>
